...
{
  /*
  printf(" \n Entering fix constructor \n"); // dbg_JMM
  printf("\n Size of: int %i | long int %i | long long %i | bigint %i  \n", sizeof(int), sizeof(long), sizeof(long long), sizeof(bigint));
  printf("\n Size of: u int %i | u long int %i | u long long %i  \n", sizeof(unsigned int), sizeof(unsigned long), sizeof(unsigned long long));
  printf("\n Max of : int %i | long %li | long long %lli \n" , INT_MAX, LONG_MAX, LLONG_MAX);
  printf("\n Max of : u int %u | u long %lu | u long long %llu | u bigint %ld \n" , UINT_MAX, ULONG_MAX, ULLONG_MAX, INT64_MAX);
  */
#if 0
  ...
  //  printf(" \n arg1 %s | arg2 %s | arg3 %s | arg4 %s | arg5 %s \n ", arg[0],arg[1], arg[2], arg[3], arg[4]); // dbg_JMM
  //  printf("\n narg: %i\n", narg); // dbg_JMM
  iarg = 3;
  prefix = strdup(arg[iarg]);
  iarg++;

  //  printf("\n prefix from iarg = 3 : %s ", prefix); // dbg_JMM
  //  printf(" \n xprd %f | yprd %f | zprd %f \n ",domain->xprd, domain->yprd, domain->zprd); // dbg_JMM
  gfmdlog = NULL;
  ...
  // This is integer division, seems to be hard coded for ndof = 3*n
  // gives zero for ndof = 1
  nu = ndof/3;
  //  printf("\n ndof %i | nu %i \n", ndof,nu); // dbg_JMM
  ...
  if (reset_map) {
    ...
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
        ...
        rnu    = std::max(rnu,   iu);
	//printf("\n ind %i | nx_loc %i | ny_loc %i | ix %i | iy %i | iu %i | rnu %i \n",i, nx_loc,ny_loc,ix,iy,iu,rnu); // dbg_JMM 
      }
    }
  ...
  //  printf(" \n xloloc %i | yloloc %i | xhiloc %i | yhiloc %i | nx_loc %i ny_loc %i | nxy_loc %i \n ", xlo_loc, ylo_loc,
  // 	  xhi_loc, yhi_loc, nx_loc, ny_loc, nxy_loc); // dbg_JMM
...
void FixGFMD::init()
{
  ...
  // Changing ndof/nu = 3 as defined above
  // if (u_xy == NULL) memory->create(u_xy, rnu*3, nxy_loc, "FixGFMD:u_xy");
  ...
  // Changing ndof/nu = 3 as defined above
  // if (f_xy == NULL) memory->create(f_xy, rnu*3, nxy_loc, "FixGFMD:f_xy");
...
void FixGFMD::pre_force(int vflag)
{
  ...
  //  printf(" \n nlocal %i | nghost %i | nall %i \n",nlocal, nall-nlocal,nall); // dbg_JMM
  ...
  */
  ...
  if (domain->triclinic == 0) {  // for orthogonal lattice
	...
	//printf("index info before %i %i \n", ix,iy); // dbg_JMM
	
        if (dxshift != 0 || dyshift != 0) {
	  //printf("\n dx shift not zero: %i %i |||||",ix,iy); // dbg_JMM
	...
	//	printf("index info after %i %i \n", ix,iy); // dbg_JMM
	  ...
	  //printf(" pre_force displacements 1: index %i | ux %f | uy %f | uz %f \n",i,ux, uy,uz); // dbg_JMM
	  ...
	  //printf(" pre_force displacements 2: index %i | ux %f | uy %f | uz %f \n",i ,ux, uy,uz); // dbg_JMM
...
{
  //  printf(" \n Entering min pre force \n"); // dbg_JMM
  pre_force(vflag);
  //  printf(" \n Leaving min pre force \n"); // dbg_JMM
}
...
{
  //  printf(" \n Entering min post force \n"); // dbg_JMM
  post_force(vflag);
  //  printf(" \n Leaving min post force \n"); // dbg_JMM
}
...
void FixGFMD::check_grid_indices()
{
  ...
  if (me == 0) {
    ...
    //printf("\n xshift of ref %i | yshift of ref %i \n ",xshift_of_ref_atom,yshift_of_ref_atom); // dbg_JMM
  ...
  //  double check_value_x = nx/xprd; // dbg_JMM
  //  double check_value_y = ny/yprd; // dbg_JMM
  
  //  printf("\n nx/xprd %f | ny/yprd %f \n",check_value_x,check_value_y); // dbg_JMM
  ...
  for (int i = 0; i < nall; i++) {
    if (mask[i] & groupbit) {
      ...
      //  printf("\n xeq %f | yeq %f | nx %i | ny %i | xprd %f | yprd %f \n",xeq[i][0],xeq[i][1],nx,ny,xprd,yprd); // dbg_JMM
      ...
      //      printf("i %i | ix %i | iy %i | ixeq %i | iyeq %i | dix %i | diy %i \n",i, ix,iy,ixeq,iyeq,dix,diy); // dbg_JMM
