...
tail -16 LJ2D_prim.log > ESC.LJ2D1.dat
...
#
echo -e 'plot "ESC.LJ2D1.dat" u ($0/8*pi):5 w lp pt 6 t "{/Symbol F}_{xx}",\c' >> gpt.scr
echo -e '"" u ($0/8*pi):8 w lp pt 5 t "{/Symbol F}_{xy}/i",\c' >> gpt.scr
echo '"" u ($0/8*pi):11 w lp pt 7 t "{/Symbol F}_{yy}"'    >> gpt.scr
#
...
tail -8 LJ2D_comp.log > ESC.LJ2D2.dat
...
echo 'set xtics ("0" 0, "{/Symbol p}/2" pi/2,"{/Symbol p}" pi,"3{/Symbol p}/2" 1.5*pi,"2{/Symbol p}" 2*pi)' >> gpt.scr
#
echo -e 'plot "ESC.LJ2D2.dat" u ($0/4*pi):5 w lp pt 5 t "{/Symbol F}_{11}",\c' >> gpt.scr
echo -e '"" u ($0/4*pi):15 w lp pt 6 t "{/Symbol F}_{22}",\c' >> gpt.scr
echo -e '"" u ($0/4*pi):25 w lp pt 7 t "{/Symbol F}_{33}",\c' >> gpt.scr
echo '"" u ($0/4*pi):35 w lp pt 9 t "{/Symbol F}_{44}"'    >> gpt.scr
#
# plot the figure and remove intermediate files
gnuplot gpt.scr
