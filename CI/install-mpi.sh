...
case $1 in
  mpich1) set -x;
    sudo apt-get -y install -q mpich-shmem-bin libmpich-shmem1.0-dev;;
  mpich2) set -x;
    sudo apt-get -y install -q mpich2 libmpich2-3 libmpich2-dev;;
  mpich3) set -x;
    sudo apt-get -y install -q gfortran libcr0 default-jdk;
